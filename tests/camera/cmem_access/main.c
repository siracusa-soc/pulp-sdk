...
int run_test() {
    ...
    CAMERA_WRITE_REG(CAMERA_REG_EXPOSURE_LATENCY , 9);
    CAMERA_WRITE_REG(CAMERA_REG_RESET_LATENCY    , 2);
    CAMERA_WRITE_REG(CAMERA_REG_READOUT_LATENCY  , 1);
...
void test_kickoff(void *arg) {
  int ret = test_entry();
  ret = camera_compare_int(CMEM_BASE_ADDRESS, golden_data , CFG_N_ITERS, CFG_N_ROWS, CFG_N_COLS/4, MAX_N_FRAMES, MAX_N_COLS, MAX_N_ROWS) ;
  pmsis_exit(ret);
}
